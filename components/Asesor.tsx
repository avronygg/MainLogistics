...
export default function Asesor({ m }: { m: Mensajes }) {
  ...
  /* Un solo enlace para los dos accesos. El mensaje precargado importa: sin
     él la persona abre WhatsApp con un chat en blanco y tiene que explicar
     de dónde viene. Con él, del otro lado se sabe que llega del sitio. */
  ...
  return (
    ...
    >
      ...
      {/* WhatsApp directo, un toque.

          Antes estaba a dos: abrir el asesor y después elegirlo. En Chile
          WhatsApp es el canal B2B por defecto —lo tienen Sotraser, Nazar,
          Loginsa y Blue Express— y el brief de desarrollo (§7.2) lo pide
          como puerta propia para el visitante que ya decidió escribir.

          Va más chico que el avatar, 52 contra 62, para que el asesor siga
          siendo el elemento principal y esto se lea como un atajo. */}
      <a
        href={enlaceWhatsapp}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={m.asesor.whatsappDirecto}
        className="pointer-events-auto grid size-[52px] shrink-0 place-items-center rounded-full bg-[#25D366] text-white shadow-[0_6px_20px_-6px_rgb(0_0_0/0.7)] transition-transform duration-[var(--dur-estado)] ease-[var(--ease-quart)] hover:scale-[1.06] active:scale-95 motion-reduce:hover:scale-100"
      >
